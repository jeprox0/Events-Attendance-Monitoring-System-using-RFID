...
<body>
    ...
    <main id="main" class="main">
        ...
        <div class="container p-4" style="border: 2px solid #ccc; border-radius: 10px; background-color: #f9f9f9; box-shadow: 0 0 10px rgba(0,0,0,0.1);">
                ...
                <table id="example" class="table table-striped table-bordered" cellspacing="0" width="100%">
                    ...
                    <tbody>
                        @foreach($contributions as $contribution)
                            <tr>
                                <td>
                                    @if ($contribution->semester)
                                        {{ $contribution->semester->school_year }} - {{ $contribution->semester->semester }}
                                    @else
                                ...
                                <td>{{ optional($contribution->event)->event_name ?? 'None' }}</td>
                                <td class="d-flex justify-content-center align-items-center">
                                    <button type="button" class="Btn me-3" data-bs-toggle="modal" data-bs-target="#editContributionModal{{ $contribution->id }}">
                                        Edit 
                                    ...
                                    <button type="button" class="btn btn-danger" style="padding: 4px 8px; font-size: 0.9rem;" onclick="confirmDelete({{ $contribution->id }})">
                                        Delete
                                    </button>
            ...
            <!-- Add Student Modal -->
            @include('contributions.create')
            @include('contributions.edit')
